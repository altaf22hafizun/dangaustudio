        ...
        <div class="row">
            ...
            <div class="col-lg-6 col-md-12 ms-auto">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <h4 class="mb-0 text-success">{{ $karya->name }}</h4>
                    @if ($karya->stock == 'Terjual')
                    <h5 class="text-danger mb-0">TERJUAL</h5>
                    @else
                    <h5 class="text-danger mb-0">Rp {{ number_format($karya->price, 0, ',', '.') }}</h5>
                    @endif
                </div>
                <p class="fw-bold mb-3">Karya: {{ $karya->seniman->name }}</p>
                <p>{{ strip_tags($karya->deskripsi) }}</p>
